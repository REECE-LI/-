...
void camera_zhufei_init(void)
{
   ...
   // DELAY_MS(9999999);			//延时以保证摄像头上面的51完成上电初始化
	
    uart_init (UART5, 115200);	//初始换串口 配置摄像头                     
    ...
    MT9V032_CFG[7] = 0x5A;                
    //通过串口发送配置参数
    uart_putbuff(UART5,MT9V032_CFG,8);
    ...
    dma_portx2buff_init(DMA_CH0,(void *)&PTB_B0_IN , (void *)image, PTA27, DMA_BYTE1, COL, DADDR_KEEPON);  
  ...
  DMA_IRQ_EN(DMA_CH0);                                  		//允许DMA通道中断
    ...
    //port_init(PTC7, ALT1 | IRQ_FALLING | PULLDOWN);    		//行中断
    port_init(PTA29, ALT1 | IRQ_FALLING | PULLDOWN);       		//场中断，上拉，下降沿触发中断，带滤波
